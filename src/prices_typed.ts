...
function createApp(database: Database) {
  ...
  const isTemporalMonday = (date: Temporal.PlainDate) => date.day === 1;

  function isMonday(date: Date) {
    return date.getDay() === 1;
  }

  const isTemporalHoliday = (date?: Temporal.PlainDate): boolean => {const holidays: Holiday[]  = database.getHolidays(); return (holidays.find((holiday: Holiday) => Temporal.PlainDate.from(holiday.holiday) === date)) ? true : false;}
